'''
New clustering & linking stage (collectively referred to as "labeling")
'''
...
from ephys2.pipeline.transform.stages import STAGES as transform_stages
from ephys2.pipeline.cluster.stages import STAGES as cluster_stages
from ephys2.pipeline.link.stages import STAGES as link_stages

class LabelStage(ProcessingStage):
	...
	def parameters() -> Parameters:
		return {
			'transform': PipelineParameter(
				units = None,
				available_stages = transform_stages,
				input_type = npt.NDArray[np.float32],
				output_type = npt.NDArray[np.float32],
				description = 'Feature transform pipeline to use before clustering and linking'
			),
			'cluster': StageParameter(
				units = None,
				available_stages = cluster_stages,
				input_type = npt.NDArray[np.float32],
				output_type = Labeling,
				description = 'Clustering algorithm'
			),
			'link': StageParameter(
				units = None,
				available_stages = link_stages,
				input_type = LinkCandidates,
				output_type = EVIncidence,
				description = 'Linking algorithm'
			),
			'link_in_feature_space': BoolParameter(
				units = None,
				description = 'Whether to link in feature space or in amplitude space'
			),
	...
	def initialize(self):
		self.cfg['transform'].initialize()
		self.cfg['cluster'].initialize()
		self.cfg['link'].initialize()
	...
	def label_and_link(self, 
		...
		block1_labels = self.cfg['cluster'].process(block1_y) 
		...
		block2_labels = self.cfg['cluster'].process(block2_y) 

		# Offset to position in label space
		block2_labels += labels_start + block1_labels.size 

		# Link clusters
		block1_features = block1_y if self.cfg['link_in_feature_space'] else block1
		block2_features = block2_y if self.cfg['link_in_feature_space'] else block2
		linkage = self.cfg['link'].process(LinkCandidates(
			block1_labels, block2_labels, block1_features, block2_features, label_space
		))
	...
	def feature_transform(self, X: npt.NDArray[np.float32]) -> npt.NDArray[np.float32]:
		...
		for stage in self.cfg['transform'].stages:
			Y = stage.process(Y) # Apply transforms in order
		M = Y.shape[1] # Account for possible dimension change
