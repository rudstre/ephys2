'''
Utilities for reading Intan RHD2000 header data.

Michael Gibson 23 April 2015
Modified Adrian Foy Sep 2018
Modified Anand Srinivasan Dec 2021
'''
...
def get_bytes_per_data_block(header: Dict) -> int:
    ...
    bytes_per_block += header['num_samples_per_data_block'] * 2 * header['num_amplifier_channels']

    # Auxiliary inputs are sampled 4x slower than amplifiers
    bytes_per_block += (header['num_samples_per_data_block'] / 4) * 2 * header['num_aux_input_channels']
    ...
    bytes_per_block += header['num_samples_per_data_block'] * 2 * header['num_board_adc_channels']

    # Board digital inputs are sampled at same rate as amplifiers
    if header['num_board_dig_in_channels'] > 0:
        bytes_per_block += header['num_samples_per_data_block'] * 2
...
def get_bytes_after_amp(header: Dict) -> int:
    '''
    Calculates the remaining number of bytes in a data block after amplifier data.

    See Intan datasheet https://intantech.com/files/Intan_RHD2000_data_file_formats.pdf
    '''
    num = 0
    sizeof_16 = 2
    
    if header['num_aux_input_channels'] > 0:
        num += int((header['num_samples_per_data_block'] / 4) * header['num_aux_input_channels']) * sizeof_16

    if header['num_supply_voltage_channels'] > 0:
        num += header['num_supply_voltage_channels'] * sizeof_16

    if header['num_temp_sensor_channels'] > 0:
        num += header['num_temp_sensor_channels'] * sizeof_16

    if header['num_board_adc_channels'] > 0:
        num += header['num_samples_per_data_block'] * header['num_board_adc_channels'] * sizeof_16

    if header['num_board_dig_in_channels'] > 0:
        num += header['num_samples_per_data_block'] * sizeof_16
...
def read_header(fid: BinaryIO, debug=False) -> Dict:
