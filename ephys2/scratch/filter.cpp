...
// torch::Tensor filtfilt_odd(
// 		const torch::Tensor& waveform,
//     const torch::Tensor& a_coeffs,
//     const torch::Tensor& b_coeffs,
//     const int padlen) {
//   TORCH_INTERNAL_ASSERT(padlen >= 0);

//   auto Z = at::cat({})

//    # Apply odd extension
//     X_ = torch.cat((-X[:,:padlen:-1], X, -X[:,padlen::-1]), dim=1)

//     # Normalize to [-1, 1]
//     hi, lo = X_.max(dim=1), X_.min(dim=1)
//     X_ -= lo
//     X_ /= 0.5*(hi-lo)
//     X_ -= 1
...
// 	// Forward filter
// 	Tensor Z = lfilter_core(X, a_coeffs, b_coeffs);
// 	// Backward filter
// 	Z = lfilter_core(Z.flip(-1), a_coeffs, b_coeffs);
// 	return Z;
// }

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("lfilter", &lfilter_core, "lfilter");
}