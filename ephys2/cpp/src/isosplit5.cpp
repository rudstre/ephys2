...
#include "../include/isosplit5/isocut5.h"
    ...
    )
{
    ...
    float *X_data = static_cast<float *>(X.request().ptr);
    int *y_data = static_cast<int *>(y.request().ptr);
    isosplit5_opts opts;
    if (isocut_threshold) { opts.isocut_threshold = *isocut_threshold; }
    if (min_cluster_size) { opts.min_cluster_size = *min_cluster_size; }
    if (K_init) { opts.K_init = *K_init; }
    if (refine_clusters) { opts.refine_clusters = *refine_clusters; }
    if (max_iterations_per_pass) { opts.max_iterations_per_pass = *max_iterations_per_pass; }
    if (seed) { srand(*seed); }
    return isosplit5_rec(y_data, M, N, X_data, opts);
}

bool isosplit5_rec(int* labels, bigint M, bigint N, float* X, isosplit5_opts opts)
{
    ...
    if ((opts.refine_clusters) && (K > 1)) {
        ...
        for (bigint k = 1; k <= K; k++) {
            ...
            if (inds_k.size() > 0) {
                ...
                isosplit5_rec(labels_k, M, inds_k.size(), X_k, opts2);
                for (bigint i = 0; i < (bigint)inds_k.size(); i++) {
...
/*
void isosplit5_old(bigint *labels_out,bigint M, bigint N,float *X_data,isosplit5_opts opts) {
    for (bigint i=0; i<N; i++) {
        labels_out[i]=1;
    }
    ...
    while (true) {
        ...
        printf ("Number of active y_data: %d\n",DD.get_active_labels().size());
...
/*
Hermitian matrix inverse-vector product via Cholesky decomposition.
Solves (A + eps * I)x = b where A is Hermitian, in-place with the result returned via b.
*/
void invh_solve(bigint M, float* A_data, float* b_data)
{
    Eigen::Map<Eigen::MatrixXf> A(A_data, M, M);
    Eigen::Map<Eigen::VectorXf> b(b_data, M);
    Eigen::MatrixXf I = Eigen::MatrixXf::Identity(M, M);
    // Apply Tikhonov regularization
    A += 1e-6 * I;
    Eigen::Map<Eigen::VectorXf> (b_data, M) = A.llt().solve(b);
}
...
bool merge_test(std::vector<bigint>* L12, bigint M, bigint N1, bigint N2, float* X1, float* X2, const isosplit5_opts& opts, float* centroid1, float* centroid2, float* covmat1, float* covmat2)
{
    ...
    invh_solve(M, avg_covmat.data(), V.data());
