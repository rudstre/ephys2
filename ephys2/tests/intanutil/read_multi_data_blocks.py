...
# Modified Anand Srinivasan December 2021: Add partial extraction
...
def read_data(filename: str, start: int=None, stop: int=None, do_notch_filter=False):
    ...
    with open(filename, 'rb') as fid:
        ...
        # print(f'Bytes per block: {bytes_per_block}, samples per block: {samples_per_block}')
        ...
        start_block = 0 if start == None else (start // samples_per_block)
        stop_block = num_data_blocks if stop == None else math.ceil(stop / samples_per_block)
        if not (start_block <= stop_block <= num_data_blocks):
            raise ValueError(f'Requested blocks {start_block} to {stop_block}, but only {num_data_blocks} total are available.')

        num_data_blocks = stop_block - start_block
        ...
        if data_present:
            ...
            # Skip to start block
            fid.seek(start_block * bytes_per_block, 1)
            ...
            if start == 0 and start == num_data_blocks:
    ...
    if (data_present):
        ...
        if do_notch_filter and header['notch_filter_frequency'] > 0 and header['version']['major'] < 3:
    ...
    # Postprocess start/stop offsets
    N = result['t_amplifier'].size
    start_offset = 0 if start == None else (start % samples_per_block)
    stop_offset = 0 if stop == None else (samples_per_block - (stop % samples_per_block))
    result['t_amplifier'] = result['t_amplifier'][start_offset:N-stop_offset]
    result['amplifier_data'] = result['amplifier_data'][:, start_offset:N-stop_offset]
