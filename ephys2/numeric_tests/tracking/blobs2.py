...
class Benchmark(TrackingBenchmark):
	eps_perturb: float = 0.5 # Maximum coordinate-wise perturbation

	def get_data_and_ground_truth(self) -> Tuple[npt.NDArray[float], npt.NDArray[np.int64]]:
		return sklearn.datasets.make_blobs(n_samples=1000, cluster_std=[1.0, 2.5, 0.5], random_state=170)
	...
	def run_algorithms(self, 
		...
		make_stage = lambda tmax: SPCSegFuseStage({
			'block_size': N,
			't_min': 0,
			't_max': tmax,
			'n_temps': 6,
		...
		result = {
			'spc_segfuse (tmax: 0.001)': make_stage(0.001).label_and_link(X, 0),
			'spc_segfuse (tmax: 0.01)': make_stage(0.01).label_and_link(X, 0),
			'spc_segfuse (tmax: 0.1)': make_stage(0.1).label_and_link(X, 0),
		}
