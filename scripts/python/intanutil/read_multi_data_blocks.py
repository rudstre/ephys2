...
def read_data(filename: str, start: int=None, stop: int=None, do_notch_filter=True):
    ...
    with open(filename, 'rb') as fid:
        ...
        stop_block = num_data_blocks if stop == None else (stop // samples_per_block + 1)
        if not (start_block <= stop_block <= num_data_blocks):
            raise ValueError(f'Requested blocks {start} to {stop}, but only {num_data_blocks} total are available.')
