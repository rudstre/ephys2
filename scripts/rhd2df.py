'''
Extract amplifier data from RHD files as CSV-formatted DataFrame for quick-and-dirty processing and analysis.

Uses https://intantech.com/downloads.html?tabSelect=Software&yPos=256 for Python interface; see modified version in `intanutil`.
...
def rhd2df(fp: str, start: int, stop: int) -> pd.DataFrame:
	data = read_data(fp, start=start, stop=stop, do_notch_filter=False)
	pdb.set_trace()
	df = pd.DataFrame(
